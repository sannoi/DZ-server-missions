class TransportMission extends SurvivorMissions
{
	...
	{		
		...
		m_MissionMessage1 = SurvivorName +", un chico que trabajaba para mí, me dijo que había cruzado una variedad especial de cannabis de varias plantas de 'Cannabis ruderalis'. Solo lo necesito para mis experimentos con los infectados. Dijo que fertilizó las plantas madre con infectado triturado.";
		m_MissionMessage2 = "Creo que se refería a cualquier tipo de infusión fermentada con agua, cenizas infectadas y trozos de carne hechos en una batidora de cocina. El impacto en el crecimiento y la floración de las plantas fue masivo. También parece que contiene una sustancia distinta a los cannabinoides nativos. Quiero extraer esta sustancia para ver cómo actúa sobre los infectados.";
		m_MissionMessage3 = SurvivorName +" ha escondido sus hierbas en un garaje de\n** "+ m_MissionLocation +" **\nCiertamente ha cerrado las puertas del garaje, así que intenta usar una ganzúa o dispararles si no tienes ninguna.";
					...
					{	
						...
						for ( int i = 0; i < CargoCount; i++ )
						{
							...
							if ( FoundObjects >= ReqWeedAmount )
							{
								...
								if(m_PlayersInZone)
								{
									for (int habi =0; habi < m_PlayersInZone.Count(); habi++)
									{
										GetHeroesAndBandits().NewPlayerAction(m_PlayersInZone.Get(habi), "SMMHeroMission");
									}
								}
